...
export const TagsInput = React.forwardRef<HTMLDivElement, TagsInputProps>(({ children, value, onValueChange, placeholder, maxItems, minItems, className, dir, ...props }, ref) => {
  ...
  const handleKeyDown = React.useCallback(
    async (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      switch (e.key) {
        ...
        case "Delete":
          if (value.length > 0) {
            if (activeIndex !== -1 && activeIndex < value.length && value[activeIndex]) {
              RemoveValue(value[activeIndex])
              moveCurrent()
            } else {
              if (target.selectionStart === 0) {
                const string = value[value.length - 1]
                if ((selectedValue === inputValue || isValueSelected) && string) {
                  RemoveValue(string)
                }
              }
            }
    ...
    >
      <div
        ...
        className={cn(
          ...
          " placeholder:text-muted-foreground selection:bg-primary border selection:text-primary-foreground dark:bg-input/30 border-input ",
          className
        )}
      >
